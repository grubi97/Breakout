...
ABall::ABall()
{
	...
	SM_Ball->SetCollisionProfileName(TEXT("PhysicsActor"));//etting inside blueprinteditor
...
void ABall::Launch()
{
	if (!BallLaunched)
	{
		...
		SM_Ball->AddImpulse(FVector(140.0f, 0.0f, 130.0f), FName(), true);//which part of bidy we want to add,
		BallLaunched = true;
	}
}
